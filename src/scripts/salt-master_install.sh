...
if [ "$solaris" = true ]; then
    cd ${SALT_PREFIX}/..
    tar -cf - salt/bin/salt* salt/bin/spm salt/dist |gzip -c >${SALT_PREFIX}/salt-master-${LATEST}-`uname -s`-`uname -r`-`uname -p`.tar.gz
else
    cd ${SALT_PREFIX}/..
    tar -cf - salt/bin/salt* salt/bin/spm salt/dist |gzip -c >${SALT_PREFIX}/salt-master-${LATEST}-`uname -s`-`uname -p`.tar.gz
fi
